...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const relMatch = text.match(/(?:关系链|Relation Chains):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-queries - full extraction, no limit
    const subQSection = text.match(/### (?:分析的子问题|Analysis Sub-queries)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible: single platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display when non-placeholder
          ...
          // No platform markers (legacy format), use whole as answer
    ...
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary \& Key Insights)\n([\s\S]*?)$/)
    if (summaryMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift Query Analysis Sub-queries'),
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (only show when both have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 heading (## xxx), section title already displayed in outer layer
  ...
  // Clean <br> before paragraph start tag immediately after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single <ol> items are separated by paragraph content, keep numbering incrementing
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('error_cn')) return 'error'
  if (log.includes('WARNING') || log.includes('warning_cn')) return 'warning'
...
// Extract final answer content - from LLM response to get section content
