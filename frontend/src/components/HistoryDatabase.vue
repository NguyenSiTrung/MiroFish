        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Start Simulation" and Step 5 "Deep Interaction" must be launched while running and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Use debounce for state transitions to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before execution, avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.grid-pattern {
  ...
  /* Position from top-left, height changes only extend at bottom, doesn't affect existing grid */
