...
const renderGraph = () => {
  ...
  // Track which edge number for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Ensures all edges distribute under same reference frame, won't overlap due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster isolated node groups toward center
  ...
  const getLinkPath = (d) => {
    ...
    // More edges = larger offset proportion of distance
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Restart simulation only when real drag is first detected
          d._isDragging = true
