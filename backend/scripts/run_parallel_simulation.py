...
Usage:
    ...
    python run_parallel_simulation.py --config simulation_config.json --no-wait  # Close immediately after done
...
# Fix encoding issues in OASIS third-party lib file reads
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Affects all open() calls without encoding specified
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
    
    # Reconfigure stdout for UTF-8 (fix console encoding)
    ...
    # Note: must be set at Python startup, runtime setting may not work
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() function, default to UTF-8 for text mode
        Fixes encoding issues in third-party libs (e.g. OASIS) reading files without specifying encoding
        """
...
# Scripts are fixed at backend/scripts/ directory
...
def disable_oasis_logging():
    ...
    OASIS logging is too verbose (records every agent observation and action), we use our own action_logger
    """
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with results, or dict with error
        """
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview item)
                - "twitter": Interview Twitter platform only
                - "reddit": Interview Reddit platform only
                - None/不指定: Interview each Agent on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    从 simulation_config 中fetched agent_id -> entity_name 的映射
    
    这样可以在 actions.jsonl 中显示真实的Entity name，而不是 "Agent_0" 这样的代号
...
def fetch_new_actions_from_db(
    ...
    """
    从数据库中fetched新的动作记录，并补充完整的上下文信息
    
    Args:
        db_path: 数据库File path
        last_rowid: 上次读取的最大 rowid 值（使用 rowid 而不是 created_at，因为不同平台的 created_at 格式不同）
        agent_names: agent_id -> agent_name 映射
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: 动作列表，每元素包含 agent_id, agent_name, action_type, action_args（含上下文信息）
...
def _enrich_action_context(
    ...
    try:
        ...
        # 引用帖子：补充原帖内容、作者和引用评论
            ...
            # fetched引用帖子的评论内容（quote_content）
            if new_post_id:
                cursor.execute("""
        ...
        # 屏蔽用户：补充被屏蔽用户的名称
            ...
            # 从 action_args 中fetched user_id 或 target_id
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    - 加速配置（Optional）：LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME
...
async def run_twitter_simulation(
    ...
    # 从配置Filefetched Agent 真实名称映射（使用 entity_name 而非Default的 Agent_X）
    agent_names = get_agent_names_from_config(config)
    # 如果配置中没有某 agent，则使用 OASIS 的Default名称
    ...
    if initial_posts:
        ...
        if initial_actions:
            ...
            log_info(f"已发布 {len(initial_actions)} 条Initial posts")
    ...
    for round_num in range(total_rounds):
        # 检查是否Received exit signal
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"Received exit signal，在第 {round_num + 1} 轮停止模拟")
            break
    ...
    # 注意：不Close environment，保留给Interview使用
    ...
    log_info(f"模拟循环done! 耗时: {elapsed:.1f}seconds, 总动作: {total_actions}")
...
async def main():
    ...
    # 在 main 函数开始时创建 shutdown 事件，确保整程序都能响应退出信号
    ...
    log_manager.info(f"模拟Parameters:")
    log_manager.info(f"  - 总Simulation duration: {total_hours}小时")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"模拟循环done! 总耗时: {total_elapsed:.1f}seconds")
    
    # 是否Entering command-wait mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-wait mode - 环境保持运行")
...
def setup_signal_handlers(loop=None):
    ...
    持久化模拟场景：Simulation done后不退出，等待 interview 命令
    当收到终止信号时，需要：
    1. 通知 asyncio 循环退出等待
    2. 让程序有机会正常Clean up resources（关闭数据库、环境等）
    3. 然后才退出
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # 设置事件通知 asyncio 循环退出（让循环有机会Clean up resources）
            if _shutdown_event:
                _shutdown_event.set()
        
        # 不要直接 sys.exit()，让 asyncio 循环正常退出并Clean up resources
        # 如果是重复Received signal，才强制退出
        else:
            print("强制退出...")
            sys.exit(1)
