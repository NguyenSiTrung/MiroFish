...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of "Agent_0" codes
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last read max rowid value (using rowid not created_at due to format differences across platforms)
    ...
    try:
        ...
        # Avoids created_at format difference issues (Twitter uses int, Reddit uses datetime string)
        cursor.execute("""
...
def _enrich_action_context(
    ...
    """
    Add context info for action (post content, usernames, etc.)
    ...
    try:
        ...
        # Block user: add blocked user name
        ...
        # Post comment: add commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Set camel-ai required env vars
    if llm_api_key:
        os.environ["OPENAI_API_KEY"] = llm_api_key
    
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key, please set LLM_API_KEY in .env file")
...
async def run_twitter_simulation(
    ...
    # If config missing an agent, use OASIS default name
    ...
    for round_num in range(total_rounds):
        ...
        # Log round start regardless of active agent count
        if action_logger:
    ...
    log_info(f"Simulation loop done! Time elapsed: {elapsed:.1f}seconds, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses accelerated LLM config (if available, fallback to generic)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation done, do not enter command-wait mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at main start, ensure entire program responds to exit signal
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop done! Total time elapsed: {total_elapsed:.1f}seconds")
...
def setup_signal_handlers(loop=None):
    """
    Set signal handlers, ensure correct exit on SIGTERM/SIGINT
    
    Persistent simulation: do not exit after simulation done, wait for interview commands
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean multiprocessing resource tracker (prevent exit warnings)
        try:
