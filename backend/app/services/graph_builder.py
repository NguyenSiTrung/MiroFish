...
class GraphBuilderService:
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to complete (by querying each episode processed status)"""
        ...
        if progress_callback:
            progress_callback(f"Starting wait for {total_episodes} text chunks to process...", 0)
        ...
        while pending_episodes:
            ...
            if progress_callback:
                progress_callback(
                    f"Zep processing... {completed_count}/{total_episodes} done, {len(pending_episodes)} pending ({elapsed}seconds)",
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with nodes and edges, including time, attributes, and detailed data
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
