...
class SimulationRunState:
    ...
    # Platform done status (detected via simulation_end event in actions.jsonl)
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Includes recent action details"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Add max rounds to command line if specified
            if max_rounds is not None and max_rounds > 0:
            ...
            # Fixes encoding issues in third-party libs (e.g. OASIS) that read files without specifying encoding
            env = os.environ.copy()
            ...
            # Set working dir to simulation dir (database files generated here)
            # Use start_new_session=True to create new process group for os.killpg termination
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr goes to same file
                text=True,
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.info(f"Simulation done: {simulation_id}")
            else:
                state.runner_status = RunnerStatus.FAILED
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If both platforms ran, both must be done
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine platform enablement by checking if corresponding actions.jsonl exists
        ...
        # If platform enabled but not done, return False
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        # File list to delete (including database files)
        files_to_delete = [
            "run_state.json",
            "simulation.log",
            "stdout.log",
        ...
        # Directory list to delete (containing action logs)
        ...
        logger.info(f"Simulation log cleanup done: {simulation_id}, deleted files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        Called on server shutdown, ensures all subprocesses are terminated
        """
        ...
        # Check if cleanup needed (avoid printing useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        Called at Flask startup, ensures cleanup of all simulation processes on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (actual running process)
        # WERKZEUG_RUN_MAIN=true indicates reloader subprocess
        # If not debug mode, this env var won't exist, still need to register
        ...
        try:
            ...
            logger.warning("Cannot register signal handlers (not main thread), using atexit only")
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get simulation environment detailed status info
