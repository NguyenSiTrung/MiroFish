    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter garbage content with question numbers
                skip = False
                for d in '123456789':
...
class ZepToolsService:
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all graph edges (paginated, with temporal info)
    ...
    def get_entity_summary(
        ...
        # First search for info related to entity
    ...
    def get_simulation_context(
        ...
        Comprehensive search for all info related to simulation requirement
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval that auto-decomposes questions with multi-dimensional search:
        1. Uses LLM to decompose questions into multiple sub-queries
        2. Performs semantic search for each sub-query
        3. Extracts related entities and gets their details
        ...
        # Step 3: Extract related entity UUIDs from edges, only get these entities (not all nodes)
        entity_uuids = set()
        ...
        # Get all related entity details (no limit, full output)
        entity_insights = []
        node_map = {}  # For later relation chain building
        
        for uuid in list(entity_uuids):  # Process all entities, no truncation
            if not uuid:
                continue
            try:
                # Get each related node info individually
    ...
    def _generate_sub_queries(
        ...
        Decompose complex questions into independently searchable sub-queries
        """
...
Return a JSON formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        3. Classify and organize active vs historical info
        
        Suitable for scenarios requiring full event overview and evolution tracking.
    ...
    def interview_agents(
        ...
        Call the real OASIS interview API to interview currently running Agents:
        1. Auto-read profile files to understand all simulated Agents
        2. Use LLM to analyze needs and intelligently select the most relevant Agents
        3. Use LLM to generate interview questions
        4. Call /api/simulation/interview/batch for real interviews (dual-platform)
        5. Integrate all results and generate interview report
        
        [IMPORTANT] Requires simulation environment to be running (OASIS not shut down)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question by number, start each with \"Question X:\" (X is the question number)\n"
        ...
        try:
            ...
            api_result = SimulationRunner.interview_agents_batch(
                ...
                platform=None,  # No platform specified, dual-platform
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                clean_text = re.sub(r'问题\d+[：:]\s*', '', clean_text)  # Remove Chinese 'Question N:' prefix
...
Please select at most {max_agents} most suitable Agents for interview and explain your reasoning."""
...
4. Objective and neutral, no bias
...
- Can use **bold** for keywords, but no other Markdown syntax"""
