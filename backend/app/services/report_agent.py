...
class ReportLogger:
    ...
    def log(
        ...
        Args:
            ...
            details: Detail dict, no truncation
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        """Record context info acquired during planning"""
        self.log(
            action="planning_context",
            stage="planning",
            details={
                "message": "Acquired simulation context info",
                "context": context
            }
        )
    ...
    def log_section_content(
        ...
        """Record section content generated (content only, not entire section complete)"""
        self.log(
...
1. Auto-decompose your question into multiple sub-queries
2. Retrieve info from simulation graph across multiple dimensions
3. Integrate results from semantic search, entity analysis, and relation chain tracking
...
[Returns]
- Related facts in original text (can be cited directly)
- Core entity insights
- Relation chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Broad Search - Get Panoramic View]
This tool gets the full panoramic view of simulation results, especially for understanding event evolution. It will:
1. Get all related nodes and relationships
2. Distinguish active facts from historical/expired facts
3. Help you understand how opinions evolved

[Use Cases]
- Need to understand complete event development
- Need to compare opinion changes across stages
...
Lightweight quick retrieval tool, suitable for simple direct queries.

[Use Cases]
- Need to quickly find specific info
...
This is NOT LLM simulation — it calls the real interview API to get original answers from simulated Agents.
Defaults to simultaneous interviews on Twitter and Reddit for more comprehensive perspectives.

Feature flow:
1. Auto-read profile files to understand all simulated Agents
2. Intelligently select Agents most relevant to the topic (e.g., students, media, officials)
3. Auto-generate interview questions
4. Call /api/simulation/interview/batch for real dual-platform interviews
5. Integrate all results for multi-perspective analysis

[Use Cases]
- Need perspectives from different roles (students? media? officials?)
- Need to collect opinions and positions from multiple parties
- Need real answers from simulated Agents (from OASIS environment)
- Want more vivid reports with "interview records"

[Returns]
- Interviewee Agent identity info
...
[Core Concept]
We have built a simulated world and injected a specific "simulation requirement" as a variable. The evolution results of the simulated world are predictions of what might happen in the future. What you are observing is not "experimental data" but "future previews".

[Your Task]
Write a "Future Prediction Report" answering:
1. Under conditions we set, what happened in the future?
...
- ✅ Focus on predictions: event trajectory, group reactions, emergent phenomena, potential risks
- ✅ Agent behaviors in simulated world ARE predictions of future human behavior
- ❌ NOT an analysis of current real-world status
- ❌ NOT a generic opinion survey

[Section Count Limit]
- Minimum 2 sections, maximum 5 sections
- No sub-sections needed, each section is written as complete content
- Content should be concise, focused on core prediction findings
- Section structure is self-designed based on prediction results

Please output a JSON-formatted report outline as follows:
{
    "title": "Report title",
    "summary": "Report summary (one-sentence core prediction finding)",
    "sections": [
        {
            "title": "Section title",
...
Variable injected into simulated world (simulation requirement): {simulation_requirement}
...
Please review this future preview from the "God's Eye View":
...
[Reminder] Section count: 2-5, content should be concise and focused on core prediction findings."""

# ── Section generation prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are an expert writer of "Future Prediction Reports", currently writing one section.
...
and Agent behaviors and interactions in the simulation ARE predictions of future human behavior.
...
❌ Do NOT write it as an analysis of current real-world status
...
1. [Must Call Tools to Observe Simulated World]
   - You are observing the future preview from "God's Eye View"
   ...
   - Call tools at least 3 times (max 5) per section to observe the simulated world representing the future
...
3. [Language Consistency - Citations Must Be Translated to Report Language]
   - Tool-returned content may contain English or mixed Chinese-English text
   - If simulation requirements and source material are in Chinese, the report must be entirely in Chinese
   - When citing English or mixed content from tools, translate it fluently into the report language
   - Preserve original meaning during translation, ensure natural and fluent expression
...
4. [Faithfully Present Prediction Results]
   ...
   - If information is insufficient, state it honestly
...
- Each section is the report's minimum content unit
- ❌ Do NOT use any Markdown headings (#, ##, ###, #### etc.) within sections
- ❌ Do NOT add section title at content start
- ✅ Section titles are auto-added by system, just write body content
- ✅ Use **bold**, paragraph breaks, quotes, lists — but NO headings

[Correct Example]
```
This section analyzes the event's opinion spread. Through deep simulation data analysis, we found...

**Initial Viral Phase**

Social media as the first scene of public opinion, served as the core channel for initial information release:

> "Social media contributed 68% of initial voice volume..."

**Emotion Amplification Phase**

Social media further amplified the event impact:
...
### I. Initial Phase     ← WRONG! Don't use ### for subsections
#### 1.1 Detailed Analysis   ← WRONG! Don't use #### for subdivision
...
- insight_forge: Deep insight analysis, auto-decomposes questions and searches facts/relations multi-dimensionally
- panorama_search: Panoramic search, understand full picture, timeline and evolution
- quick_search: Quick verification of specific info point
...
Each reply you can do only ONE of the following (not both):

Option A - Call a tool:
Output your thinking, then call a tool in this format:
<tool_call>
...
When you have enough info from tools, output section content starting with "Final Answer:".

⚠️ Strictly Prohibited:
- Do NOT include both tool calls and Final Answer in one reply
- Do NOT fabricate tool results (Observation), all results are injected by system
- Max one tool call per reply
...
4. [Quote Format Rules - Must Be Separate Paragraphs]
   ...
   ✅ Correct format:
   ```
   The official response was considered lacking substance.

   > "The official response pattern appears rigid and sluggish in the fast-paced social media environment."
   ...
   ❌ Incorrect format:
   ```
   The response was lacking substance.> "The response pattern..." This assessment reflects...
   ```
5. Maintain logical coherence with other sections
6. [Avoid Repetition] Carefully read completed sections below, do not repeat the same information
7. [Reminder] Do NOT add any headings! Use **bold** instead of subsection headings"""
...
- ✅ Section titles are auto-added by system
- ✅ Write body directly, use **bold** instead of subsection headings

Please begin:
1. First think (Thought) about what info this section needs
...
- If info is sufficient: output section content starting with "Final Answer:" (must cite above text)
- If more info needed: call a tool to continue retrieval
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[NOTE] You have only called tools {tool_calls_count} times, minimum {min_tool_calls} required."
    "Please call more tools for simulation data before outputting Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only {tool_calls_count} tool calls, minimum {min_tool_calls} required."
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately output section content starting with "Final Answer:" based on acquired info.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used: {unused_list}, try different tools for multi-angle info"

REACT_FORCE_FINAL_MSG = "Tool call limit reached, please output Final Answer: and generate section content."
...
- Lead with conclusions, then explain reasons"""
...
class ReportAgent:
    ...
    # Valid tool names for bare JSON fallback validation
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try if format 1 didn't match, to avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflicts (tool call + Final Answer)
        ...
        for iteration in range(max_iterations):
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        ...
                        "content": (
                            ...
                            "Each reply can only do one of these:\n"
            ...
            if has_final_answer:
                # Insufficient tool calls, reject and require more
            ...
            if has_tool_calls:
                # Tool quota exhausted → clearly inform, require Final Answer
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM tried {len(tool_calls)} tools, executing only first: {call['name']}")
            ...
            # Use this content directly as final answer, no more idle spinning
        ...
        logger.warning(f"Section {section.title} max iterations reached, force generating")
        ...
        if response is None:
            logger.error(f"Section {section.title} forced finalization LLM returned None, using default error")
    ...
    def generate_report(
        ...
        Each section is saved immediately after generation, no need to wait for full report.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Console output logs during report generation (INFO, WARNING, etc.),
        different from the structured agent_log.jsonl logs.
        
        Args:
            report_id: Report ID
            from_line: Start line for reading (incremental, 0 = from beginning)
    ...
    def save_section(
        ...
        Called after each section is generated, enabling section-by-section output
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines that duplicate section title at start
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Section titles are system-added, content should not have any headings
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        Returns info on all saved section files
        """
        folder = cls._get_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-processing: clean up heading issues in entire report
            ...
            if heading_match:
                ...
                if level == 1:
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Non-section h2 headings to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compat: check files stored directly in reports directory
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # Backward compat: delete individual files
        deleted = False
