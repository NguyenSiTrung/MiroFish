...
Endpoint 1: Analyze text content, generate entity and relationship type definitions for social simulation
"""
...
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze given text content and simulation requirements, designing entity types and relationship types suitable for **social media opinion simulation**.
...
- We need to simulate reactions and information propagation paths of all parties in opinion events
...
Your 10 entity types must include these levels:

A. **Fallback types (must include, placed at end of list)**:
   - `Person`: Fallback for any individual. Used when a person does not fit other specific person types.
   - `Organization`: Fallback for any organization. Used when an org does not fit other specific org types.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for key roles appearing in the text
   - e.g.: For academic events, use `Student`, `Professor`, `University`
   - e.g.: For business events, use `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people appear in text, e.g. "school teachers", "bystanders", "anonymous netizens"
- Without a specific type match, they should fall under `Person`
...
5. Attribute names cannot use reserved words (name, uuid, group_id), use full_name, org_name, etc.
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                ...
                # Remove from end (preserve more important specific types)
