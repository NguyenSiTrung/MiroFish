...
def prepare_simulation():
    """
    Prepare simulation environment (async task, LLM-generated parameters)
    ...
    Features:
    - Auto-detects completed preparation, avoiding duplicate generation
    ...
    try:
        ...
        # ========== Synchronously fetch entity count (before background task) ==========
        # This lets frontend get expected agent count immediately after calling prepare
        try:
...
def get_prepare_status():
    ...
    Response:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether preparation exists
    ...
    try:
        ...
        if not task:
            # Task not found, but if simulation_id provided, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    returns the latest one if multiple exist (sorted by created_at)
...
def get_simulation_history():
    ...
    Used for homepage history display, returns simulation list with project name, description, etc.
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - agent_configs: Agent activity config (activity level, posting frequency, stance, etc.)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // Optional: update Agent activity to Zep graph memory
            "force": false                         // Optional: force restart (stops running simulation and cleans logs)
        }

    About force parameter:
        - When enabled, stops running/completed simulations and cleans run logs
        - Cleaned items: run_state.json, actions.jsonl, simulation.log, etc.
    ...
    About enable_graph_memory_update:
        ...
        - This lets the graph "remember" the simulation, for subsequent analysis or AI conversation
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # Process not found or ended, reset status to ready
...
def get_run_status(simulation_id: str):
    """
    Get simulation run status in real-time (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows only latest round content from both platforms
...
def interview_agent():
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // If not specified: dual-platform interview
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure the simulation has completed and entered command-wait mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // No platform = use default
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by per-item platform)
                                               // If not specified: dual-platform interview for each Agent
...
def close_simulation_env():
    ...
    Send close-environment command to gracefully exit command-wait mode.
